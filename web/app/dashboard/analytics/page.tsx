...
import { CategoryVolumeChart, ResolutionSplitChart } from "@/components/AnalyticsCharts";
...
export default async function AnalyticsPage() {
  ...
  const categoryData = Object.entries(byCategory).map(([name, count]) => ({ name, count }));
  ...
  return (
    <div>
      <h1 className="text-xl font-medium text-ink-950 mb-1">Phân tích chất lượng</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-ink-700/10 p-5">
          ...
          <div className="text-2xl font-medium text-signal-600 mt-1">{autoRate}%</div>
        </div>
        <div className="bg-white rounded-xl border border-ink-700/10 p-5">
          <div className="text-xs font-mono uppercase tracking-wide text-ink-700/60">
            Đã chuyển nhân sự
          </div>
          <div className="text-2xl font-medium text-flare-600 mt-1">{humanCount}</div>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-ink-700/10 p-5">
          <h2 className="text-sm font-medium text-ink-950 mb-3">Khối lượng theo danh mục</h2>
          <CategoryVolumeChart data={categoryData} />
        </div>
        <div className="bg-white rounded-xl border border-ink-700/10 p-5">
          <h2 className="text-sm font-medium text-ink-950 mb-3">Tự động so với nhân sự</h2>
